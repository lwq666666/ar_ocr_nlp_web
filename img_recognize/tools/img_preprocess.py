...
from config import IMG_SIZE, LARGEST_NUMBER_OF_SYMBOLS,SCALSIZE


#读取图片并将图片转化成二值图,返回原彩色图和二值图
def read_img_and_convert_to_binary(original_img):
    #读取待处理的图片
    # original_img = cv2.imread(filename)
    # print(original_img)
    #将原图分辨率缩小SCALSIZE倍，减少计算复杂度original_img.shape[1]
    if original_img.shape[1]<400:
    ...
    ret, binary_img_new = cv2.threshold(img_gray, 255,120 , cv2.cv2.THRESH_TRUNC)
    ret,binary_img = cv2.threshold(img_gray,120,1,cv2.THRESH_BINARY_INV+cv2.THRESH_OTSU)
    return original_img,binary_img,binary_img_new
...
def extract_img(location,img,contour=None):
    ...
    # else:
    res = cv2.resize(extracted_img, (IMG_SIZE, IMG_SIZE), interpolation=cv2.INTER_AREA)
    d = int(abs(res.shape[0] - res.shape[1]) / 2)
...
def binary_img_segment(binary_img,binary_img_new,original_img=None):
    ...
    for contour in contours:
        ...
        # 只提取轮廓内的字符
        extracted_img = extract_img(location,binary_img_new,contour)
...
# original_img, binary_img =read_img_and_convert_to_binary('/Users/liweiqiang/PycharmProjects/imgtotext/math4.jpg')
