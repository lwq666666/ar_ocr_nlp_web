import numpy as np
from config import SPACIAL_RELATIONSHIP as spatial_relationship
from img_recognize.tools.img_preprocess import extract_img
        ...
        for j in range(IMG_SIZE):
            for k in range (IMG_SIZE):
                if one[j][k] == True:
                    ...
                    t[j][k] = 0
        symbols_to_be_predicted.append(np.array(t))
    return np.array(symbols_to_be_predicted)

def normalize_matrix_value_new(symbol_segment_list):
    symbols_to_be_predicted = []
    for i in range(len(symbol_segment_list)):
        t = np.ones((IMG_SIZE,IMG_SIZE), np.uint8)
        for j in range(IMG_SIZE):
