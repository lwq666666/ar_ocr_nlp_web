...
tf.app.flags.DEFINE_string("char_dict", "img_recognize/char_dict.txt", "path to character dict")
...
def recongnize(img_list):
    ...
    with tf.Session() as sess:
        ...
        for i in range(len(index)):
            predictions.append({'symbol':fileToNumpy('img_recognize/char_dict.txt')[int(index[i][0])],'probabilities':val[i][0]})
